...
def parse_args():
    ...
    training_group.add_argument("--seed", type=int, default=7)
    ...
    ##### @Moh_7596: DSV4 sanity check args ###############
    sglang_group.add_argument(
        "--sglang-load-format",
        type=str,
        default="auto",
        choices=["auto", "dummy"],
        help="SGLang weight loading format. 'dummy' = random weights from config.json only.",
    )
    sglang_group.add_argument(
        "--sglang-json-model-override-args",
        type=str,
        default="{}",
        help='JSON string to override model config, e.g. \'{"num_hidden_layers": 2}\'',
    )
    ###############
...
def build_models(args) -> Tuple[DFlashTargetModel, DFlashDraftModel]:
    ...
    if args.target_model_backend == "sglang":
        ...
        ##### @Moh_7596: forward load_format and model overrides to SGLang ###############
        load_format = getattr(args, "sglang_load_format", "auto")
        target_model_kwargs["load_format"] = load_format
        if load_format == "dummy":
            print_on_rank0(
                "[SANITY CHECK] Using --load-format dummy: "
                "real architecture, random weights, no checkpoint download"
            )

        json_override = getattr(args, "sglang_json_model_override_args", "{}")
        if json_override != "{}":
            target_model_kwargs["json_model_override_args"] = json_override
            print_on_rank0(f"[SANITY CHECK] Model override: {json_override}")
        ###############
...
##### @Moh_7596: fallback for config-only (dummy) mode ###############
def _load_target_embeddings_and_head(args):
    """Load target embeddings and lm_head.

    In dummy mode (config-only, no safetensors), create random-init
    embeddings matching the target config.
    """
    load_format = getattr(args, "sglang_load_format", "auto")

    # Also detect config-only directory (no safetensors)
    import glob, os as _os
    _has_weights = bool(
        glob.glob(_os.path.join(args.target_model_path, "*.safetensors")) +
        glob.glob(_os.path.join(args.target_model_path, "pytorch_model*.bin"))
    )

    if load_format == "dummy" or not _has_weights:
        print_on_rank0(
            "[SANITY CHECK] No safetensors on disk — creating random "
            "embeddings and lm_head from target config"
        )
        ...
        embed_tokens = torch.nn.Embedding(vocab_size, hidden_size).to(
            device="cuda", dtype=torch.bfloat16
        )
        lm_head = torch.nn.Linear(hidden_size, vocab_size, bias=False).to(
            device="cuda", dtype=torch.bfloat16
        )
        ...
        components.lm_head = lm_head
        print_on_rank0(
            f"[SANITY CHECK] Random embed_tokens: [{vocab_size}, {hidden_size}], "
            f"lm_head: [{hidden_size}, {vocab_size}]"
        )
        return components
    else:
...
def main():
    ...
    ##### @Moh_7596: use fallback-aware loader for embeddings ###############
    print_on_rank0("Loading target embeddings and head...")
    target_components = _load_target_embeddings_and_head(args)
    ###############
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        for step_in_epoch, data in enumerate(progress_bar):
            ...
            if dist.get_rank() == 0:
                ...
                ##### @Moh_7596 — per-step METRIC line for compare_runs.py
                _world = dist.get_world_size()
                _tokens_global = int(attention_mask.sum().item()) * _world

                # NPU memory (falls back to CUDA if not on Ascend)
                if hasattr(torch, "npu") and torch.npu.is_available():
                    _mem_alloc = torch.npu.memory_allocated() / 1e9
                    _mem_max   = torch.npu.max_memory_allocated() / 1e9
                else:
                    _mem_alloc = torch.cuda.memory_allocated() / 1e9
                    _mem_max   = torch.cuda.max_memory_allocated() / 1e9

                _cumulative_tokens += _tokens_global
                metric = {
                    "type": "step",
                    ...
                    "tokens_global": _tokens_global,
                    "step_time_s": float(elapsed),
                    "tokens_per_sec": (_tokens_global / elapsed) if elapsed > 0 else None,
                    "mem_alloc_gb": _mem_alloc,
                    "mem_max_gb":   _mem_max,
                    "world_size":   _world,
            ...
            ##### @Moh_7596 — periodic empty_cache to fight fragmentation
        ...
        ##### @Moh_7596 — end-of-epoch METRIC + reset
        if dist.get_rank() == 0:
            epoch_metric = {
                "type": "epoch",
                "epoch": int(epoch),
                "epoch_time_s": time.perf_counter() - _epoch_start,
                "cumulative_tokens": _cumulative_tokens,
                "global_step": int(global_step),
            }
            print(f"METRIC {json.dumps(epoch_metric)}", flush=True)
        _epoch_start = time.perf_counter()  # reset for next epoch
        ###############
