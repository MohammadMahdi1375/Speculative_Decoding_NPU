...
class MQALayer(nn.Module):
    ...
    def _forward_prepare_multi_stream(
        ...
        with torch.cuda.stream(stream_kv):
            ...
            ##### @Moh_7596 — guard store_cache (AscendAttnBackend lacks it).
            # V4 has an optimization that pre-stores KV before attention via
            # `store_cache`. The NPU backend doesn't implement this method;
            # skipping is safe -- the normal cache write path still runs in
            # forward_core.
            if self.overlap_store_cache and hasattr(attn_backend, "store_cache"):
            ################
                ...
                try:
                    ...
                    for param_name, weight_name, shard_id in stacked_params_mapping:
                        ...
                        if name not in params_dict and name.startswith("mtp"):
                            break
                        ##### @Moh_7596 — skip unmapped stub weights instead of KeyError.
                        # SGLang's V4 loader has graceful skips for experts/.bias/mtp* but
                        # the catch-all path here raises on names that don't exist in the
                        # constructed model (common with stub weights or fused-name mismatches).
                        # Downgrade to warn+skip so smoke tests can run.
                        if name not in params_dict:
                            logger.warning(
                                f"[@Moh_7596 stub] skipping unmapped weight {name}"
                            )
                            continue
                        ################
