...
def fused_rope(
    ...
    ##### @Moh_7596 — NPU fallback for fused_rope.
    # The CUDA JIT path below needs CUDA_HOME to compile; on Ascend NPU
    # we route to apply_rotary_emb_triton (a @triton.jit kernel that runs
    # under triton-ascend). Same in-place semantics on q and k.
    try:
...
def _dispatch_bf16_fp32_backend(
    ...
    ##### @Moh_7596 — On NPU, force PyTorch fallback regardless of algo.
    # The "cublas" path JIT-compiles a CUDA extension (needs CUDA_HOME);
    # "deep_gemm" needs CUDA too. PyTorch fallback is the only option on NPU.
    if x.is_npu or y.is_npu or (hasattr(x.device, "type") and x.device.type == "npu"):
